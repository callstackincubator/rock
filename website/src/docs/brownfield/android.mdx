...
1. Add the gradle plugin dependency to your `android/build.gradle`:
   ...
   buildscript {
       ...
       dependencies {
           classpath("com.callstack.react:brownfield-gradle-plugin:0.6.2") // check the latest version: https://mvnrepository.com/artifact/com.callstack.react/brownfield-gradle-plugin
       }
   }
   ```
...
dependencies {
    // Match your version of React Native, here 0.86:
    api("com.facebook.react:react-android:0.86.0")
    // For React Native 0.86 or newer use:
...
The template defaults `newArchEnabled=true` in `android/gradle.properties`. Set it to `false` if you need to opt out of the New Architecture.
